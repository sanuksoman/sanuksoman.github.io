...
var Gallery = (function() {
	...
	Room.prototype = {
		...
		create : function() {
			...
			if( this.totalItems > 1 ) {
				this.$navPrev = $( '<span class="gr-prev">prev</span>' ).bind( 'click', $.proxy( this.navigate, this, 'prev' ) );
				this.$navNext = $( '<span class="gr-next">next</span>' ).bind( 'click', $.proxy( this.navigate, this, 'next' ) );
			...
			this.$caption.find( 'span.gr-caption-close' ).bind( 'click', $.proxy( this.hideDescription, this ) );

			// click on item's caption
			var self = this;
			this.$el.bind( 'click', 'figure > figcaption', function() {
				var $caption = $( this ),
					$item = $caption.parent(), 
					idx = $item.index() - 1;
			...
			// click items
			this.$el.bind( 'click', 'figure > div', function() {
		...
		changeWall : function( dir ) {
			...
			setTimeout( $.proxy( function() {
				...
				} ).bind( transEndEventName, $.proxy( function() {
					
					// set original width
					this.$auxWall.unbind( transEndEventName ).css( 'width', this.$auxWall.data( 'width' ) );
		...
		jump : function( pos, callback ) {
			...
			var jumpnow = $.proxy( function() {
				...
				var	$item = this.walls[ this.currentWall ].$items.eq( this.currentItem ),
					...
					afterAnim = function() {
						this.$mainWall.unbind( transEndEventName );
						this.walking = false;
						if( callback ) {
							callback.call();
						}
				...
				if( support.transitions && support.transforms3d ) {
					this.$mainWall.css( 'transform', transformVal ).bind( transEndEventName, $.proxy( afterAnim, this ) );
				}
				else {
		...
		hideDescription : function( callback ) {
			...
			var hidedescfn = $.proxy( function() {
				this.$caption.unbind( transEndEventName );
				this.caption = -1;
			...
			if( support.transitions ) {
				this.$caption.bind( transEndEventName, hidedescfn );
			}
			else {
				hidedescfn.call();
			}
	...
	function initEvents() {		
		$( window ).unbind( 'debouncedresize' ).bind( 'debouncedresize', function() {
			winsize = getWindowSize();
